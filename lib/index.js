...
class CertBase {
  ...
  listSignedCerts() {
    ...
    return _asyncToGenerator(function* () {
      let certs = [];
      const files = yield readDir(_this4.certsDirPath);

      if (files.length > 0) {
        certs = files.filter(function (name) {
  ...
  removeCert(hostname) {
    ...
    return _asyncToGenerator(function* () {
      const folderPath = path.join(_this6.certsDirPath, hostname);
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!name) {
        const keyFileData = {
          ...
          path: _this8.caKeyPath
        };
        const certFileData = {
          content: certData.certificate,
          path: _this8.caCertPath
        };
        ...
        const keyFileData = {
          content: certData.clientKey,
          path: _this8._makePath(name, 'key')
        };
        const certFileData = {
          content: certData.certificate,
          path: _this8._makePath(name, 'cert')
        };
